    ...
    function _ins() {
        ...
        this.custom_data = function() {
            ...
            for (var i = 0; i < l; i++) {
                ...
                if (clicky_custom.visitor) {
                    ...
                    if (temp) {
                        data[key] = temp;
                        if (clicky_custom.visitor_consent) _self.set_cookie('_custom_data_' + key, temp, 30 * 86400);
                    }
                }
        ...
        this.start_monitors = function() {
            if (!monitors) {
                monitors = 1;
                _self.hm_monitor();
                if (!window._htmlvid && clicky_custom.html_media_track && (document.getElementsByTagName('audio').length || document.getElementsByTagName('video').length)) {
                ...
                if (!clicky_custom.history_disable && window.history && window.history.pushState) {
                    ...
                    _self.add_event(window, 'popstate',
                    function(e) {
        ...
        this.beacon = function(type, q, called_by_pageview) {
                    ...
                    for (var i in clicky_custom['split']) {
                        if (clicky_custom['split'].hasOwnProperty && clicky_custom['split'].hasOwnProperty(i)) {
                            if (i == 'goal' && typeof clicky_custom['split'].goal == 'object') {
                                for (var j = 0,
                                l = clicky_custom['split'].goal.length; j < l; j++) {
            ...
            for (var site_id_index = 0; site_id_index < site_ids.length; site_id_index++) {
                ...
                if (type != 'pageview' && (window['NO_PINGY_' + site_id] || _self.get_cookie('unpoco_' + site_id))) continue;
                ...
                _self.inject(_self.domain + '/in.php?site_id=' + site_id + '&type=' + type + q + custom + goal + split + (jsuid ? '&jsuid=' + jsuid: '') + (_self.get_cookie('unpoco_' + site_id) ? '&upset': '') + (_self.get_cookie('heatmaps_g2g_' + site_id) ? '&hmset': '') + (clicky_custom.visitor_consent ? '&consent=1': '') + '&mime=js&x=' + Math.random(), (type == 'pageview' ? 'js': 'beacon'));
            }
            if (navigator.userAgent.match(/msie|trident/i) && (type == 'outbound' || type == 'download')) _self.pause();
            _self.ref = '';
            _self.utm = '';
            _self.ping_start();
        };
